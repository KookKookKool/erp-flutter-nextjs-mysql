  ...
  try {
    ...
    const employeesQuery = `
      ...
      LEFT JOIN hrm_employees e ON (u.id = e.user_id ${
        columnNames.includes("employee_id")
          ? "OR u.employee_id = e.employee_code"
          : ""
      })
    ...
    const employees = (results as any[]).map((row: any) => ({
      ...
      first_name:
        row.emp_first_name || row.first_name || row.name?.split(" ")[0] || "",
      last_name:
        row.emp_last_name ||
        row.last_name ||
    ...
    try {
      const createEmployeeQuery = `
        INSERT INTO hrm_employees (
          ...
          email, phone, position, level, hire_date, status, 
